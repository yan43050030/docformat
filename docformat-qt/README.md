...
- **附件一致性检查**：正文的"附件：1.xxx 2.yyy"说明与文末的"附件1/附件2"标识两侧对照——
  个数对不对得上、序号连不连续有无重复、只有一个附件却编了号（GB/T 9704 规定单个附件不编号）。
  附件常是单独的文件，正文有说明而本文档没标识的情形只作提示，不算差错
- **归档命名与登记表**：文号、标题、文种、成文日期、密级、份号这些字排版时已经认过，
  按可自定义的命名式（如 `{成文日期}-{文号}-{标题}`）批量改名归档，每行可再手改；
  默认**复制**到归档目录（原件不动）、重名自动加 (2) 不覆盖，并把每份的信息
  追加到 CSV 登记台账（utf-8-sig，Excel 双击不乱码）。
  台账存的是明文标题与密级，请按其中最高密级选择存放位置
