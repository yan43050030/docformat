...
OWNER = {
    ...
    '承办部门 上边线距下': 'dept',
    '经办人 上边线距下': 'handler',
    # 落款**不**参与校准：它唯一的纵向实测项是"字面底边距纸下面"，
    # 而字面底边到行盒底边的距离随字体而变，本机没装方正楷体、用的是替代
    # 字体，这个目标根本够不着。让校准去追它，就会把 sign 越推越大
    # （实测推到 -1.21cm）——而落款和年月日是同一段，那一整行会被拽上去
    # 1.2cm，在用户装了真字体的机器上反而全错。宁可让这一项差着，
    # 也不能拿一个够不着的目标去带偏一整行。
}
